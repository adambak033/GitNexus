...
## Fork: betterGitNexus — tRPC & Curried-Call Support

This fork adds **tRPC pattern detection** and **curried-call resolution** to GitNexus's ingestion pipeline, enabling first-class indexing of tRPC-based architectures (router → procedure → workflow with curried signatures like `workflow(db)(input)`).

The native tool does not support curried signatures or tRPC procedures/indentation, which is the bulk of our backend code architecture. This fork fills that gap.

### What changed

| Feature | Files | Effect |
|---------|-------|--------|
| **HOC-in-pair function detection** | `languages/typescript/query.ts`, `tree-sitter-queries.ts`, `languages/typescript.ts` | tRPC procedures (`create: proc.mutation(async () => {})`) now appear as named Function nodes |
| **Curried call detection** | `languages/typescript/query.ts`, `tree-sitter-queries.ts` | `workflow(db)(input)` calls correctly resolve to the workflow function (outer call_expression on a call_expression) |
| **tRPC route extraction** | `route-extractors/trpc.ts` (new), `framework-detection.ts`, `languages/typescript.ts`, `workers/parse-worker.ts` | Individual tRPC routes detected with procedure-name prefixing; line-by-line scanner handles `.input()` chaining |
| **Function/Const dedup** | `languages/typescript/captures.ts`, `workers/parse-worker.ts` | `const fn = () => {}` no longer creates duplicate Const + Function nodes (pre-pass collects higher-priority definition startIndexes) |
| **tRPC handler resolution** | `call-processor.ts` | tRPC procedure names resolve to their Function nodes for route→process linking |

### Results (Jurialis project)

| Metric | Before | After |
|--------|--------|-------|
| tRPC procedures as Function nodes | 0 | 297 (across 42 router files) |
| tRPC routes in `route_map` | 1 (catch-all) | 256 |
| `context({name: "createTeamWorkflow"})` | ambiguous (Function + Const) | found (single Function node, 1 caller) |
| `impact({target: workflow})` | 0 callers (misleading) | Multi-hop traces working, LOW risk |
| Inter-workflow curried calls | 0 | Detected |

### Install

```bash
# Clone and link globally (replaces the official gitnexus command)
git clone https://github.com/adambak033/GitNexus.git betterGitNexus
cd betterGitNexus/gitnexus
npm install
npm run build
npm link

# Verify the fork is active (should print the fork's path, not an npm path)
which gitnexus
```

> **Note:** `gitnexus` (bare) uses the fork; `npx gitnexus` uses the official npm version. To revert: `npm unlink -g gitnexus && npm install -g gitnexus`.

### Upstream sync

This fork tracks `abhigyanpatwari/GitNexus` as `upstream` and is kept in sync. To sync after upstream changes:

```bash
git fetch upstream
git merge upstream/main
# resolve README.md conflict by taking upstream's version + re-inserting this Fork section
cd gitnexus && npm run build && npm link
```

After re-linking, re-index your projects (use `clean --force` to avoid stale parse-cache):

```bash
cd <your-project> && gitnexus clean --force && gitnexus analyze --force
```
