...
import { canonicalizePath, cloneDirBelongsToEntry, loadMeta, saveMeta, listRegisteredRepos, getStoragePath, registryPathEquals, } from '../storage/repo-manager.js';
...
export const isAllowedOrigin = (origin) => {
    ...
    if (createPublicOriginMatcher(process.env[PUBLIC_ORIGIN_ENV])?.matches(parsed))
        return true;
...
export const createServer = async (port, host = '127.0.0.1') => {
    ...
    app.post('/api/analyze/upload', createRouteLimiter({ limit: 5 }), requireTrustedOrigin, createAnalyzeUploadHandler({
    ...
    app.post('/api/embed', createRouteLimiter({ limit: 20 }), requireTrustedOrigin, async (req, res) => {
        try {
            const entry = await resolveRepo(requestedRepo(req));
            if (!entry) {
                res.status(404).json({ error: 'Repository not found' });
            ...
            (async () => {
                ...
                try {
                    ...
                    await withLbugDb(lbugPath, async () => {
                        ...
                        const forceReembedNodeIds = resume?.action === 'resume' ? resume.pendingNodeIds : new Set();
                         ...
                         */
                        const countPersistedEmbeddings = async () => {
                            const counted = await measurePersistedEmbeddingCount(executeQuery);
                            if (counted.kind === 'unknown') {
                                logger.warn({ reason: counted.reason }, '[embed] could not count persisted embeddings; leaving stats.embeddings untouched');
                            }
                            return counted;
                        };
                        ...
                        const pipelineResult = await runEmbeddingPipeline(executeQuery, executeWithReusedStatement, (p) => {
                            embedJobManager.updateJob(job.id, {
                                progress: {
                        ...
                        const outcome = resolveEmbedRunOutcome(embeddingIdentity, pipelineResult, finalizeContext);
                    ...
                    if (!current || current.status !== 'failed') {
                        ...
                        // `progress.phase` (#2790).
                        embedJobManager.updateJob(job.id, partialRunError === undefined
                            ? {
                                status: 'complete',
