 ...
 */
import { mintPartialCheckpoint } from '../core/embedding-checkpoint.js';
import { resolvePersistedEmbeddingCount, } from '../core/embedding-count.js';
/**
 ...
 */
export const resolveEmbedRunOutcome = (identity, result, context = {}) => {
    if (result.failedNodeIds.length === 0) {
        ...
        if (countIsKnown || metaAlreadyRecordsEmbeddings)
    ...
    return {
        ...
        error: `Embedding generation finished partially: ${result.failedNodeIds.length} node(s) lost ` +
 ...
 */
export const withMeasuredEmbeddingCount = (meta, 
