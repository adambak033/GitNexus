...
export function createLaunchAnalysisWorker(deps) {
    ...
    return function launchAnalysisWorker(job, targetPath, opts) {
        ...
        const workerHeapMb = Number.isInteger(envHeapMb) && envHeapMb > 0 ? envHeapMb : Math.min(8192, autoHeapCapMb());
        const forkWorker = () => {
                    ...
                    waitForSettledIndex(targetPath, jobStartMs)
                        ...
                        if (collapse) {
                            ...
                            jobManager.updateJob(job.id, {
                                ...
                                error: `Repository indexed INCOMPLETELY: only ${collapse.persisted} of ` +
