 ...
 */
export function normalizeBoundHost(boundHost) {
    if (!boundHost)
        return undefined;
    // Bracket a bare IPv6 literal so `new URL` can parse it as a host.
    const candidate = boundHost.includes(':') && !boundHost.startsWith('[') ? `[${boundHost}]` : boundHost;
    let hostname;
    try {
 ...
 */
export function createPublicOriginMatcher(rawOrigin) {
    ...
    return {
        hostname,
        matches: (origin) => origin.hostname === hostname &&
...
function splitAuthority(authority) {
    ...
    if (/[\s,;*/?#@\\]/.test(host) || /^\d+$/.test(host) || host.endsWith('.'))
        return undefined;
 ...
 */
export function createWriteOriginGuard(boundHost, boundPort) {
    ...
    return function requireTrustedOrigin(req, res, next) {
        ...
        try {
            ...
            const matchesBoundHost = hostname === normalizedBoundHost &&
 ...
 */
export function isServeAuthConfigured() {
    return false;
}
/**
 ...
 */
export function assertServeAuthForPublicOrigin() {
    const raw = process.env[PUBLIC_ORIGIN_ENV]?.trim();
    if (!raw || isServeAuthConfigured())
        return;
    throw new Error(`${PUBLIC_ORIGIN_ENV} is set (${raw}), but 'gitnexus serve' has no authentication yet. ` +
 ...
 */
export function logOriginPolicy(boundHost) {
    const raw = process.env[PUBLIC_ORIGIN_ENV]?.trim();
    const publicOrigin = createPublicOriginMatcher(raw);
    if (publicOrigin) {
        logger.info({ [PUBLIC_ORIGIN_ENV]: raw, hostname: publicOrigin.hostname }, `[gitnexus serve] Browser write routes also accept origins on ${publicOrigin.hostname}.`);
    }
    else if (raw) {
        logger.warn({ [PUBLIC_ORIGIN_ENV]: raw }, `[gitnexus serve] Ignoring ${PUBLIC_ORIGIN_ENV}=${raw} — not a single reachable origin, ` +
            `so it admits nothing. Set it to one host, optionally with a scheme and a port.`);
    }
    if (!boundHost || normalizeBoundHost(boundHost) !== undefined)
        return;
    ...
    logger.warn({ host: boundHost }, `[gitnexus serve] Bound to a wildcard address (${boundHost}); browser write routes ${admitted}`);
}
...
export function resolveTrustProxy(raw) {
    ...
    if (/^(true|yes|on)$/i.test(value)) {
        return rejectTrustProxy(value, `it trusts every hop, so req.ip is read from the client-controlled leftmost ` +
    ...
    if (/^\d+$/.test(value)) {
        ...
        if (Number.isInteger(hops) && hops <= MAX_TRUST_PROXY_HOPS)
            return hops;
 ...
 */
export function warnIfRateLimitKeysCollapse(boundHost) {
    if (process.env[TRUST_PROXY_ENV]?.trim())
        return;
    if (!boundHost)
        return;
    // normalizeBoundHost returns undefined for a wildcard or unparseable host,
    // neither of which is loopback — so both warn.
    if (isLoopbackHostname(normalizeBoundHost(boundHost)))
        return;
    logger.warn({ host: boundHost, trustProxy: DEFAULT_TRUST_PROXY }, `[gitnexus serve] Bound to ${boundHost} with ${TRUST_PROXY_ENV} unset, so 'trust proxy' is ` +
...
function rejectTrustProxy(value, reason) {
    logger.warn({ [TRUST_PROXY_ENV]: value }, `[gitnexus serve] Ignoring ${TRUST_PROXY_ENV}=${value} (${reason}); falling back to ` +
        `'${DEFAULT_TRUST_PROXY}'.`);
    return DEFAULT_TRUST_PROXY;
}
