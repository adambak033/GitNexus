...
async function getContextResource(backend, repoName) {
    ...
    const indexedRunnerSchema = freshMeta?.runnerIdentity
        ?.schemaVersion;
    lines.push(`  runner_identity_schema_status: ${JSON.stringify(indexedRunnerSchema === ANALYZER_RUNNER_IDENTITY_SCHEMA_VERSION
        ? 'current'
        : 'legacy-or-unknown')}`);
    ...
    lines.push('');
    lines.push('re_index: Run `npx gitnexus analyze --index-only` in terminal if data is stale ' +
