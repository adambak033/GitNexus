...
function parseRepositoryPolicy(env) {
    ...
    if (allowedRaw) {
        allowed = allowedRaw.value
            ...
            .filter(Boolean);
        if (allowed.length === 0)
            throw new Error(`${allowedRaw.key} must not be blank.`);
    }
    let defaultRepo;
    if (defaultRaw) {
        defaultRepo = defaultRaw.value.trim();
        if (!defaultRepo)
            throw new Error(`${defaultRaw.key} must not be blank.`);
    }
...
function resolveSpecifier(specifier, registry) {
...
function startupResolutionError(reason) {
    return new Error(reason === 'ambiguous'
...
function unavailableRepositoryError() {
...
export class McpRepositoryPolicy {
    ...
    allowedPathKeys;
    defaultRepo;
    uniqueAllowedContextNames;
    static unrestricted() {
        return new McpRepositoryPolicy([], undefined, undefined);
    }
    constructor(registry, allowed, defaultRepo) {
        ...
        this.uniqueAllowedContextNames = new Set(this.allowed
            .map((repo) => repo.name.toLowerCase())
            .filter((name) => registryNameCounts.get(name) === 1));
    }
    resolveRuntimeRepo(specifier) {
    ...
    normalizeToolArgs(args) {
        if (!this.configured)
            return args;
        if (!this.restricted && args?.repo !== undefined)
            return args;
    ...
    async listAllowedRepos(backend) {
        ...
        return current
            ...
            .map((repo) => {
            const siblings = repo.siblings?.filter((sibling) => this.allowedPathKeys.has(normalizedPath(sibling.path)));
            return {
                ...repo,
    ...
    async callTool(backend, method, params) {
        if (!this.configured)
            return backend.callTool(method, params);
        if (method === 'list_repos')
            return this.listReposPage(backend, params);
    ...
    async resolveRepo(backend, repo, branch) {
        if (!this.configured)
            return backend.resolveRepo(repo, branch);
        if (!this.restricted)
            return backend.resolveRepo(repo ?? this.defaultRepo?.path, branch);
    ...
    assertResourceUri(uri) {
        ...
        try {
            ...
            if (repoShaped)
                this.resolveRuntimeRepo(decodeURIComponent(repoShaped[1]));
            return;
        }
        ...
        if (parsed.protocol.toLowerCase() !== 'gitnexus:')
            return;
        ...
        return new Proxy(backend, {
            get(target, property, receiver) {
                if (property === 'callTool') {
                    return (method, params) => policy.callTool(target, method, params);
                }
                if (property === 'listRepos')
                    return () => policy.listAllowedRepos(target);
                ...
                if (policy.restricted && (property === 'queryClusters' || property === 'queryProcesses')) {
                    return (repoName, limit) => {
                        const selected = policy.repoForArgs(repoName === undefined ? undefined : { repo: repoName });
...
export function mcpRepositoryPolicyConfigured(env = process.env) {
    const raw = parseRepositoryPolicy(env);
    return raw.allowed !== undefined || raw.defaultRepo !== undefined;
}
export async function createMcpRepositoryPolicy(backend, env = process.env) {
    const raw = parseRepositoryPolicy(env);
    ...
    if (raw.allowed) {
        ...
        for (const specifier of raw.allowed) {
            ...
            if (!result.repo)
                throw startupResolutionError(result.reason ?? 'invalid');
