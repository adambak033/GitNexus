 ...
 */
export function createMCPServer(backend, options = {}) {
    ...
    server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => {
        const templates = getResourceTemplates().filter((template) => readOnlyResourceTemplateAllowed(template.uriTemplate, readOnly) &&
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        try {
            ...
            const content = filterMcpReadOnlyResourceContent(await readResource(uri, scopedBackend), readOnly);
            return {
                contents: [
                    {
                        uri,
...
export async function startMCPServer(backend, repositoryPolicy) {
