...
import { deriveSpringBeanMetadata, } from '../../core/ingestion/frameworks/spring/bean-catalog.js';
import { decodeSpringBeanFactoryReason, } from '../../core/ingestion/frameworks/spring/bean-factories.js';
export async function queryClassBeanMetadata(lbugPath, symbolId, symbolType) {
    if (symbolType !== 'Class' && symbolType !== 'Method' && symbolType !== 'CodeElement') {
        return undefined;
    }
    try {
        if (symbolType !== 'Class') {
            const pattern = symbolType === 'Method'
            ...
            const rows = await executeParameterized(lbugPath, `${pattern}
        ...
        const rows = await executeParameterized(lbugPath, `
    ...
    `, { symbolId });
        ...
        if (!Array.isArray(value))
            return undefined;
        return deriveSpringBeanMetadata(value.filter((annotation) => typeof annotation === 'string'));
    }
    catch {
