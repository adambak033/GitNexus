...
export async function pdgLayerStatus(deps) {
    ...
    try {
        ...
        const rows = await deps.executeParameterized(deps.lbugPath, `MATCH (:BasicBlock)-[r:CodeRelation]->(:BasicBlock) WHERE r.type = 'CDG' OR r.type = 'REACHING_DEF' RETURN r.type AS type LIMIT 1`, {});
 ...
 */
const seedBlockQuery = (anchorClause, probeLimit) => `MATCH (a:BasicBlock) WHERE ${anchorClause} RETURN a.id AS id ORDER BY a.startLine, id LIMIT ${probeLimit}`;
/**
...
async function interproceduralDescent(input) {
    ...
    hopLoop: for (let hop = 0; hop < depthBudget; hop++) {
        ...
        const spanSeeds = await Promise.all(spans.map(async (span) => {
            ...
            const rawSeedRows = await exec(lbugPath, seedBlockQuery(anchorClause, probeLimit), queryParams);
...
export async function runImpactPDG(deps) {
    ...
    const rawSeedRows = await exec(repo.lbugPath, seedBlockQuery(anchorClause, probeLimit), queryParams);
    ...
    const ownerFnLine = typeof sym.startLine === 'number' ? toOneBasedLine(sym.startLine) : Number.NaN;
