...
export function resolveMcpMaxTokens(toolName, args, env = process.env) {
    if (!BUDGETED_TOOLS.has(toolName))
        return undefined;
    if (args?.maxTokens !== undefined)
        return parsePositiveInteger(args.maxTokens, 'maxTokens');
...
export function applyMcpMaxTokens(text, maxTokens) {
    if (maxTokens === undefined)
        return text;
    const textBytes = Buffer.byteLength(text, 'utf8');
    if (maxTokens >= Math.ceil(textBytes / MCP_TOKEN_ESTIMATE_BYTES))
        return text;
...
export function withoutMcpBudgetArg(args) {
    if (!args || !Object.prototype.hasOwnProperty.call(args, 'maxTokens'))
        return args;
