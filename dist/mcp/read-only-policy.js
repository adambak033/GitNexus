...
export function resolveMcpReadOnlyMode(env = process.env) {
...
export function assertMcpReadOnlyToolCall(toolName, args, readOnly) {
    if (!readOnly)
        return;
    ...
    for (const groupOnlyArg of ['crossDepth', 'subgroup']) {
        if (args?.[groupOnlyArg] !== undefined) {
            throw new Error(`Parameter "${groupOnlyArg}" is not available in GitNexus MCP read-only mode.`);
        }
    }
}
export function readOnlyResourceTemplateAllowed(uriTemplate, readOnly) {
    return !readOnly || !/^gitnexus:\/\/group\//iu.test(uriTemplate);
}
export function assertMcpReadOnlyResource(uri, readOnly) {
    if (!readOnly)
        return;
...
export function filterMcpReadOnlyResourceContent(content, readOnly) {
    if (!readOnly)
        return content;
    return content
        .split('\n')
        .filter((line) => !/^\s*-\s+(?:rename|cypher|group_sync|group_list):/u.test(line) &&
...
export function scrubGroupDescription(description) {
    return description
        .replace(/\nGROUP MODE:[\s\S]*?(?=\n\n[A-Z][A-Z ()-]*:|$)/gu, '')
        .replace(/\nCROSS-REPO \(experimental\):[\s\S]*?(?=\n\n[A-Z][A-Z ()-]*:|$)/gu, '')
        .replace(/\nDESTINATION TRACE \(cross-repo\):[\s\S]*?(?=\n\n[A-Z][A-Z ()-]*:|$)/gu, '');
}
export function toolForReadOnlyMcp(tool, readOnly) {
    if (!readOnly)
        return tool;
    ...
    if (repo && typeof repo === 'object') {
        properties.repo = {
            ...repo,
            description: 'Indexed repository name or path. Group-mode values beginning with @ are unavailable in MCP read-only mode.',
        };
    }
