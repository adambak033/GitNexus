 ...
 */
export function sanitizeDetectedBranch(value) {
    if (!value)
        return undefined;
    try {
        ...
        case 'embeddings': {
            ...
            if (typeof value === 'number') {
                if (!Number.isInteger(value) || value < 0) {
                    throw new GitNexusRcError(`${source} must be true/false or a non-negative integer (node cap; 0 disables the cap).`);
                }
                return String(value);
            }
            throw new GitNexusRcError(`${source} must be a boolean or a non-negative integer (node cap; 0 disables the cap).`);
        }
        default:
...
const normalizeLevel = (obj, { allowNestedKey }) => {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (!Object.hasOwn(KEY_SPECS, key)) {
            throw new GitNexusRcError(`Unknown key "${key}" in ${GITNEXUS_RC_FILENAME}. ${ALLOWED_KEYS_HINT}`);
        }
        const spec = KEY_SPECS[key];
        const prev = setBy.get(spec.target);
        if (prev && prev !== key) {
            throw new GitNexusRcError(`${GITNEXUS_RC_FILENAME}: "${prev}" and "${key}" both configure the same option; set only one.`);
        }
 ...
 */
export function loadAnalyzeConfig(repoRoot) {
    ...
    try {
        ...
        throw new GitNexusRcError(`${GITNEXUS_RC_FILENAME} is not valid JSON: ${err.message}. ` +
 ...
 */
export function mergeAnalyzeOptions(cli, config) {
    if (!config)
        return cli;
    const merged = { ...cli };
    for (const key of Object.keys(config)) {
        if (key === 'stats' || key === 'defaultBranch')
            continue; // handled below / by resolver
