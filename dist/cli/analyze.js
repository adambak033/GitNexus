...
async function ensureHeap() {
    ...
    if (ambientHeapMb !== null) {
        if (ambientHeapMb >= RESPAWN_HEAP_MB)
            return false;
        cliWarn(`  NODE_OPTIONS pins the heap to ${ambientHeapMb}MB — below the ${RESPAWN_HEAP_MB}MB this machine's RAM supports.\n` +
...
const analyzeCommandImpl = async (inputPath, cliOptions, runnerIdentityAtBootstrap) => {
    ...
    try {
        ...
        const selfCommitSafety = options.selfCommit === true
