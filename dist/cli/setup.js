...
async function setupOpenCode(result) {
    ...
    try {
        ...
        if (ok) {
            ...
            result.errors.push(`OpenCode: ${path.basename(configPath)} is corrupt — skipping to preserve existing content`);
        }
    }
    catch (err) {
...
// ─── Skill Installation ───────────────────────────────────────────
export const RENAMED_SKILL_DIRS = {
 ...
 */
export const LEGACY_SKILL_DIR_NAMES = Object.values(RENAMED_SKILL_DIRS).flat();
/**
        ...
        try {
            ...
            for (const oldName of RENAMED_SKILL_DIRS[skillName] ?? []) {
                ...
                if (await dirExists(legacyDir)) {
                    console.log(`[gitnexus] skill "${oldName}" was renamed to "${skillName}"; ` +
