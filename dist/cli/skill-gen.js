...
export const generateSkillFiles = async (repoPath, projectName, pipelineResult) => {
    ...
    if (mirrorToAgents) {
        try {
            ...
            await Promise.all(entries
                .filter((entry) => entry.isDirectory() && entry.name.startsWith(GENERATED_SKILL_PREFIX))
                .map((entry) => fs.rm(path.join(agentsOutputDir, entry.name), { recursive: true, force: true })));
        }
        catch {
    ...
    if (mirrorToAgents) {
        try {
            ...
            console.log(`Warning: Could not create mirror root ${agentsOutputDir} — .agents/skills mirroring disabled for this run: ${err}`);
    ...
    if (mirrorToAgents) {
        console.log(`  ${skills.length} skills mirrored \u2192 .agents/skills/${GENERATED_SKILL_PREFIX}*/ (.agents)`);
    }
    return { skills, outputPath: outputDir };
};
// ============================================================================
