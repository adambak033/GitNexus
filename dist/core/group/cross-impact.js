...
export function mergeRisk(localRisk, cross) {
    ...
    if (highConf)
        return 'HIGH';
    if (traversed.length > 0 && (localRisk === 'LOW' || localRisk === 'UNKNOWN'))
        return 'MEDIUM';
    return localRisk;
}
/**
...
function addCrossImpact(cross, candidate) {
    ...
    if (candidate.fanout_status === 'not_attempted') {
        if (cross.some(sameBoundary))
            return;
    }
    else {
        const boundaryOnlyIndex = cross.findIndex((entry) => sameBoundary(entry) && entry.fanout_status === 'not_attempted');
        if (boundaryOnlyIndex >= 0)
            cross.splice(boundaryOnlyIndex, 1);
    }
