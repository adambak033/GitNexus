...
function declarationAnnotations(node) {
    ...
    if (!modifiers)
        return [];
    return modifiers.namedChildren.filter((child) => child.type === 'annotation' || child.type === 'marker_annotation');
}
...
function scanRouteAnnotations(tree) {
    ...
    for (const { captures } of matches) {
        ...
        if (node.type === 'method_declaration') {
            ...
            if (httpMethods.length > 0) {
                const feignHttpMethod = httpMethods.length === 1 ? (httpMethods[0] === '*' ? 'GET' : httpMethods[0]) : null;
    ...
    const classHttpMethodsByTypeId = new Map();
    ...
    const constrainedMethodRoutes = methodRoutes.flatMap((route) => {
        const typeNode = findEnclosingInterface(route.methodNode) ?? findEnclosingClass(route.methodNode);
