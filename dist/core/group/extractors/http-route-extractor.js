 ...
 */
function resolveContainingSymbol(rows, line) {
    ...
    const pick = (probe) => {
        ...
        for (const r of rows) {
            ...
            if (!['Function', 'Method', 'CodeElement'].some((l) => labels.includes(l)))
                continue;
...
function resolveSymbolByName(rows, name) {
    const norm = (x) => String(x ?? '');
    for (const r of rows) {
        const labels = JSON.stringify(r.labels ?? r[5] ?? '');
        if (!['Function', 'Method', 'CodeElement'].some((l) => labels.includes(l)))
            continue;
        if (norm(r.name ?? r[1]) !== name)
            continue;
        const uid = norm(r.uid ?? r[0]);
        if (uid)
            return { uid, name, filePath: norm(r.filePath ?? r[2]) };
    }
    return null;
}
...
export class HttpRouteExtractor {
    ...
    async extract(dbExecutor, repoPath, _repo) {
        ...
        const loadFileSymbols = async (filePath) => {
            if (!dbExecutor)
                return [];
        ...
        const graphProviders = dbExecutor != null
            ? await this.extractProvidersGraph(dbExecutor, getDetections, resolveDetectionSymbol, coveredFiles)
            : [];
        ...
        const providers = this.mergeGraphAndSourceContracts(graphProviders, await this.extractProvidersSourceScan(scanFiles, getDetections, resolveDetectionSymbol));
        const graphConsumers = dbExecutor != null
            ? await this.extractConsumersGraph(dbExecutor, getDetections, resolveDetectionSymbol)
            : [];
        const consumers = this.mergeGraphAndSourceContracts(graphConsumers, await this.extractConsumersSourceScan(scanFiles, getDetections, resolveDetectionSymbol));
    ...
    async extractProvidersGraph(db, getDetections, resolveSymbol, coveredFiles) {
        const out = [];
    ...
    async extractProvidersSourceScan(files, getDetections, resolveSymbol) {
        const out = [];
        for (const rel of files) {
    ...
    async extractConsumersGraph(db, getDetections, resolveSymbol) {
        const out = [];
        let rows;
        try {
        ...
        for (const row of rows) {
            ...
            const resolved = consumerCandidates.length === 1
    ...
    async extractConsumersSourceScan(files, getDetections, resolveSymbol) {
        const out = [];
        for (const rel of files) {
            const detections = await getDetections(rel);
            const filePath = normalizeRepoRelPath(rel);
