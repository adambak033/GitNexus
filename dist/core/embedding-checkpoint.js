 ...
 */
export const checkpointKind = (checkpoint) => checkpoint.kind ?? 'interrupted';
/** Pending-node count, tolerating the field's optionality. */
export const pendingNodeCount = (checkpoint) => checkpoint.pendingNodeIds?.length ?? 0;
/**
 ...
 */
export const nextAttemptCount = (resumedFrom, failedNodeIds) => {
    if (resumedFrom === undefined || checkpointKind(resumedFrom) !== 'partial')
        return undefined;
 ...
 */
export const mintInterruptedCheckpoint = (identity, progress, pendingNodeIds, at = new Date().toISOString()) => ({
    at,
    ...progress,
 ...
 */
export const mintPartialCheckpoint = (identity, result, resumedFrom, at = new Date().toISOString()) => ({
    at,
 ...
 */
export const mintUnverifiedCountCheckpoint = (identity, progress, at = new Date().toISOString()) => ({
    at,
    ...progress,
    model: identity.model,
    dimensions: identity.dimensions,
 ...
 */
export const decideEmbeddingResume = (checkpoint, identity, options = {}) => {
    ...
    if (options.force) {
        return {
            action: 'discard',
            log: 'Discarding the embedding checkpoint (--force)' +
    ...
    if (kind === 'partial' && (checkpoint.attempts ?? 0) >= maxAttempts) {
        return {
            action: 'abandon',
            log: `Warning: ${pending} node(s) failed to embed on ${checkpoint.attempts} consecutive ` +
    ...
    const identityDiffers = providerDiffers ||
    ...
    if (identityDiffers && kind !== 'interrupted') {
        ...
        return {
            action: 'abandon',
            log: `Warning: dropping ${pending} pending node(s) from a ${kind} embedding checkpoint: it ` +
    ...
    if (providerDiffers) {
        return {
            action: 'abort',
            error: 'Cannot resume embedding checkpoint: the embedding provider configuration differs. ' +
    ...
    if (identityDiffers) {
        return {
            action: 'abort',
            error: `Cannot resume embedding checkpoint: it uses ${checkpoint.model} at ` +
    ...
    return {
        action: 'resume',
        log: `Previous analyze ended at an embedding checkpoint (${checkpoint.nodesProcessed}/` +
