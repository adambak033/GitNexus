 ...
 */
export function buildSupertypeAlternation(descriptor, tag) {
    const seen = new Set();
    const unique = [];
 ...
 */
const INNER_NAME_FIELDS = ['name', 'type', 'property', 'attribute', 'value'];
/**
...
export const SUPERTYPE_NODE_TYPE_SETS = {
    ...
    get skippedInnerTypes() {
        return SKIPPED_INNER_TYPES;
    },
    get leadingNameTypes() {
 ...
 */
const SKIPPED_INNER_TYPES = new Set([
    'type_arguments',
    'type_argument_list',
 ...
 * can be added here.
 */
const LEADING_NAME_TYPES = new Set(['explicit_delegation']);
 ...
 */
export function normalizeSupertypeName(node) {
    return normalize(node, 0);
}
function normalize(node, depth) {
    if (!node || depth > MAX_NORMALIZE_DEPTH)
        return '';
    ...
    for (let i = node.namedChildCount - 1; i >= 0; i--) {
        ...
        if (SKIPPED_INNER_TYPES.has(child.type))
            continue;
        const inner = normalize(child, depth + 1);
        if (inner.length > 0)
            return inner;
 ...
 */
export function simplifyRawName(text) {
