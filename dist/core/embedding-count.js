 ...
 */
import { EMBEDDING_TABLE_NAME } from '../_shared/index.js';
 ...
 */
export const measurePersistedEmbeddingCount = async (runQuery) => {
    let rows;
    try {
    ...
    if (Number.isFinite(parsed))
        return { kind: 'measured', count: parsed };
    return {
        kind: 'unknown',
...
export const persistedEmbeddingCountOrUndefined = (result) => (result.kind === 'measured' ? result.count : undefined);
/**
 ...
 */
export const resolvePersistedEmbeddingCount = (measured, priorCount) => (measured.kind === 'measured' ? measured.count : priorCount);
