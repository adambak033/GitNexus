 ...
 */
import { closeSync, constants as fsConstants, existsSync, fstatSync, lstatSync, mkdirSync, openSync, readSync, readFileSync, readdirSync, readlinkSync, realpathSync, renameSync, statSync, unlinkSync, writeFileSync, } from 'node:fs';
...
export const ANALYZER_RUNNER_IDENTITY_SCHEMA_VERSION = 4;
const BUILD_CANONICALIZATION = 'gitnexus-analyzer-build-v2';
const DEPENDENCY_RUNTIME_CANONICALIZATION = 'gitnexus-analyzer-dependency-runtime-v4';
...
function detectLibcVariant() {
    ...
    try {
        ...
        if (Array.isArray(report?.sharedObjects)) {
            ...
            if (musl)
                return `musl:${path.basename(musl)}`;
        }
    }
    catch {
...
function hashStableFile(candidate) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
            ...
            if (!isDeepStrictEqual(openedBefore, before.target))
                continue;
            const hash = createHash('sha256');
 ...
 * `canonicalizePath`.
 */
export function normalizeAnalyzerRootPath(p, platform) {
    if (platform !== 'win32')
        return p;
...
function resolveDependencyPackageRoot(fromRoot, packageName, pathGuards, limits) {
    ...
    while (true) {
        ...
        if (isFile(manifestPath))
            return resolveExistingPath(path.dirname(manifestPath));
        const parent = path.dirname(cursor);
        if (parent === cursor)
            return null;
        cursor = parent;
...
export function _clearAnalyzerIdentityProcessCacheForTests() {
...
function snapshotCacheGuards(requests) {
    ...
    try {
        const probe = spawnSync(process.execPath, ['--input-type=commonjs', '-e', CACHE_GUARD_PROBE_SCRIPT], {
...
function validateIdentityCache(cache, options) {
    ...
    const mismatch = actual.findIndex((result, index) => !isDeepStrictEqual(result, entries[index][1]));
    if (mismatch !== -1) {
...
function persistIdentityCache(packageRoot, buildRoot, payload, previous, options) {
    if (previous && isDeepStrictEqual(previous, payload))
        return;
...
function isAnalyzerRunnerIdentity(value) {
    ...
    return (identity.schemaVersion === ANALYZER_RUNNER_IDENTITY_SCHEMA_VERSION &&
        !!runtime &&
 ...
 */
export function normalizeAnalyzerRunnerIdentityForComparison(identity) {
...
export function resolveAnalyzerRunnerIdentity(analyzerModuleUrl, options = {}) {
 ...
 */
export function analyzerRunnerIdentitiesEqual(indexedIdentity, currentIdentity) {
 ...
 */
export async function captureAnalyzerIdentityBeforeLoad(analyzerModuleUrl, loader, options = {}) {
...
export function finalizeAnalyzerRunnerIdentity(analyzerModuleUrl, startedWith, options = {}) {
