 ...
 */
export function detectGraphWriteCollapse(expected, 
/**
...
persisted) {
    ...
    if (expectedCount < GRAPH_WRITE_COLLAPSE_MIN_EDGES)
        return { verdict: 'healthy' };
    if (persistedCount >= expectedCount * GRAPH_WRITE_COLLAPSE_RATIO)
        return { verdict: 'healthy' };
    return { verdict: 'collapsed', expected: expectedCount, persisted: persistedCount };
}
/** Stable machine-readable reasons an index cannot be certified complete. */
export function getIndexIncompleteReasons(meta) {
    const reasons = [];
    if (meta?.incrementalInProgress)
        reasons.push('incremental-in-progress');
    ...
    if (meta?.embeddingCheckpoint) {
        ...
        // repo-manager.ts).
        reasons.push(checkpointKind(meta.embeddingCheckpoint) === 'unverified-count'
            ? 'embedding-count-unverified'
            : 'embedding-checkpoint-pending');
    }
    return reasons;
