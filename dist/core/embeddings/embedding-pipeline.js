 ...
 */
const queryFallbackFileNodes = async (executeQuery) => {
    try {
...
export const batchInsertEmbeddings = async (executeWithReusedStatement, updates) => {
    ...
    if (paramsList.length === 0)
        return;
    await executeWithReusedStatement(`MATCH (e:${EMBEDDING_TABLE_NAME} {id: $id}) DELETE e`, paramsList.map(({ id }) => ({ id })));
...
const minSubBatchesBeforeFailureRatioGuard = (totalNodes, subBatchSize) => Math.min(MAX_SUB_BATCHES_FLOOR, Math.max(MIN_SUB_BATCHES_FLOOR, Math.ceil(totalNodes / subBatchSize / RATIO_GUARD_SAMPLE_FRACTION_DIVISOR)));
/**
 ...
 */
const failureRatioAbortError = (failures, attempted, cause) => new Error(`[embed] Aborting: ${failures} of ${attempted} embed sub-batches failed ` +
    ...
    `Underlying failure: ${cause instanceof Error ? cause.message : String(cause)}`, { cause });
/**
 ...
 */
const abortWithFailedCleanupError = (abortErr, cleanupErr) => new Error(`${abortErr instanceof Error ? abortErr.message : String(abortErr)} — additionally, ` +
    ...
    `${cleanupErr instanceof Error ? cleanupErr.message : String(cleanupErr)}`, { cause: abortErr });
/**
 ...
 */
const isCancellationError = (err) => typeof err === 'object' && err !== null && err.name === 'AbortError';
/**
 ...
 */
export const runEmbeddingPipeline = async (executeQuery, executeWithReusedStatement, onProgress, config = {}, skipNodeIds, existingEmbeddings, pipelineOptions = {}) => {
    ...
    const throwIfCancelled = () => pipelineOptions.signal?.throwIfAborted();
    try {
        throwIfCancelled();
        ...
        if (forceReembedNodeIds && forceReembedNodeIds.size > 0) {
            const removedPendingNodeIds = [...forceReembedNodeIds].filter((nodeId) => !embeddableNodeIds.has(nodeId));
        ...
        const checkpointWindowNodeCount = Math.max(batchSize, Math.ceil(checkpointEveryNodes / batchSize) * batchSize);
        ...
        const minSubBatchesForRatioGuard = minSubBatchesBeforeFailureRatioGuard(totalNodes, finalConfig.subBatchSize);
        ...
        for (let batchIndex = 0; batchIndex < totalNodes; batchIndex += batchSize) {
            ...
            const batchFailedNodeIds = new Set();
            ...
            for (let si = 0; si < allTexts.length; si += EMBED_SUB_BATCH) {
                ...
                try {
                    ...
                    if (pipelineOptions.signal?.aborted || isCancellationError(embedErr))
                        throw embedErr;
                    ...
                    logger.warn({ err: embedErr }, `⚠️ embedBatch failed for ${subTexts.length} texts (first: "${subTexts[0]?.substring(0, 80)}...") — ` +
                    ...
                    if (subBatchesAttempted >= minSubBatchesForRatioGuard &&
            ...
            if (batchFailedNodeIds.size > 0) {
                ...
                for (const nodeId of batchFailedNodeIds)
        ...
        if (failedNodeIds.size > 0) {
            logger.warn(`⚠️ Embedding pipeline finished with ${failedNodeIds.size} node(s) dropped after embed failures; ` +
                'their rows were removed so the next run re-embeds them.');
        }
        if (isDev) {
            logger.info(`✅ Embedding pipeline complete! (${totalChunks} chunks from ${processedNodes}/${totalNodes} nodes)`);
        }
        return {
            nodesProcessed: processedNodes,
            chunksProcessed: totalChunks,
