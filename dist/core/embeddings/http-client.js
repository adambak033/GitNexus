...
let lastHttpRequestStartedAt;
let httpPaceQueue = Promise.resolve();
const parsePositiveIntegerEnv = (name, fallback, max) => {
...
const parseNonNegativeIntegerEnv = (name, fallback, max) => {
    const raw = process.env[name];
    if (raw === undefined || raw === '')
        return fallback;
    if (!/^\d+$/u.test(raw)) {
...
const cancelledError = () => new DOMException('Embedding request cancelled', 'AbortError');
...
const paceHttpRequest = async (minIntervalMs, signal) => {
    throwIfAborted(signal);
    if (minIntervalMs <= 0)
        return;
    const waitTurn = httpPaceQueue.then(async () => {
        throwIfAborted(signal);
        const waitMs = lastHttpRequestStartedAt === undefined
            ? 0
 ...
 */
export const getHttpTimeoutMs = () => readConfig()?.timeoutMs;
/**
 ...
 */
const sanitizeReason = (reason, url, apiKey) => {
...
class RetryableEmbeddingBodyError extends Error {
    terminalMessage;
    constructor(terminalMessage, options) {
 ...
 */
const countMismatchMessage = (received, expected, safeEndpoint, batchIndex) => `Embedding endpoint returned ${received} vectors for ${expected} texts ` +
    ...
    try {
        ...
        resp = await resilientFetch(url, {
            ...
            fetchImpl: async (input, init) => {
                ...
                let payload;
                try {
                    payload = (await attemptResp.json());
                }
                catch (err) {
                ...
                if (payload.data.length !== batch.length) {
                    throw new RetryableEmbeddingBodyError(countMismatchMessage(payload.data.length, batch.length, safeUrl(url), batchIndex));
                }
                parsed = payload.data;
                return attemptResp;
            },
        ...
        if (requestOptions.signal?.aborted ||
            ...
            throw new HttpEmbeddingError(`Embedding request cancelled (${safeUrl(url)}, batch ${batchIndex})`, { cause: err });
        }
        ...
        if (err instanceof DOMException && err.name === 'TimeoutError') {
            throw new HttpEmbeddingError(`Embedding request timed out after ${timeoutMs}ms (${safeUrl(url)}, batch ${batchIndex})`, { cause: err });
        }
        ...
        throw new HttpEmbeddingError(`Embedding request failed (${safeUrl(url)}, batch ${batchIndex}): ${reason}`, { cause: safeCause });
    }
    if (!resp.ok) {
 ...
 */
export const httpEmbed = async (texts, requestOptions = {}) => {
    if (texts.length === 0)
        return [];
    ...
    for (const [batchIndex, batch] of chunk(texts, HTTP_BATCH_SIZE).entries()) {
        ...
        if (items.length !== batch.length) {
            throw new HttpEmbeddingError(countMismatchMessage(items.length, batch.length, safeUrl(url), batchIndex));
        }
 ...
 */
export const httpEmbedQuery = async (text, requestOptions = {}) => {
    const config = readConfig();
    if (!config)
        throw new Error('HTTP embedding not configured');
    const url = `${config.baseUrl}/embeddings`;
