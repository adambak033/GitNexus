 ...
 */
export function warnIfQueryTextUnbounded(cypher, context, warn) {
    ...
    if (cypher.length * 3 <= QUERY_TEXT_CEILING_BYTES)
        return;
    const bytes = Buffer.byteLength(cypher, 'utf8');
    if (bytes <= QUERY_TEXT_CEILING_BYTES)
        return;
    warn(`${context}: query text is ${Math.round(bytes / 1024)} KB. A list spliced into query ` +
