 ...
 */
const warnedKeyCounts = new Map();
const WARN_MILESTONES = [1, 10, 100, 1000, 10000];
const ordinal = (n) => {
    switch (n) {
        case 1:
            return '1st';
...
export const isMissingFsError = (err) => err?.code === 'ENOENT';
const missing = isMissingFsError;
export const sidecarPreflightDisabled = () => /^(1|true|yes|on)$/i.test(process.env.GITNEXUS_DISABLE_LBUG_SIDECAR_PREFLIGHT ?? '');
export const statIfExists = async (filePath) => {
    try {
...
const warnOnce = (logger, key, message) => {
    ...
    const isMilestone = WARN_MILESTONES.includes(next);
    if (!isMilestone) {
        logDebug(logger, message);
        return;
    }
...
export const shadowSidecarRecoveryMessage = (dbPath, err) => {
    const msg = err instanceof Error ? err.message : String(err);
    return (`LadybugDB checkpoint sidecar is missing for ${dbPath}. ` +
 ...
 */
export const renameFailureMessage = (dbPath, err) => {
...
export async function inspectLbugSidecars(dbPath) {
...
export async function quarantineWalForMissingShadow(dbPath, options) {
    ...
    const message = `GitNexus: quarantined WAL ${path.basename(quarantinePath)} because LadybugDB shadow sidecar was missing; ` +
...
export async function preflightLbugSidecars(dbPath, options) {
    let state;
    try {
        state = await inspectLbugSidecars(dbPath);
    }
    catch (err) {
        logDebug(options.logger, `GitNexus: unable to inspect LadybugDB sidecars before ${options.mode} open; continuing without preflight repair: ${err.message}`);
        return { kind: 'clean', dbPath };
    }
    if (sidecarPreflightDisabled() || !options.allowQuarantine)
        return state;
    ...
    if (state.kind === 'orphan-wal') {
        warnOnce(options.logger, `${dbPath}:orphan-wal-preflight:${options.mode}`, `GitNexus: found ${state.walBytes} byte lbug.wal without lbug.shadow before ${options.mode} open; ` +
            'will rely on LadybugDB replay/recovery instead of deleting pending WAL data.');
    }
    return state;
}
export async function finalizeLbugSidecarsAfterClose(dbPath, options) {
    ...
    try {
        ...
        logDebug(options.logger, `GitNexus: unable to inspect LadybugDB sidecars after close; skipping post-close repair: ${err.message}`);
        return;
    }
    if (state.kind === 'clean' || state.kind === 'wal-with-shadow')
        return;
    ...
    if (state.kind === 'tiny-orphan-wal') {
        try {
            ...
            if (!missing(err)) {
                warnOnce(options.logger, `${dbPath}:post-close-tiny-quarantine-failed`, `GitNexus: failed to quarantine tiny orphan WAL after close (${err.message}); next read may recover reactively.`);
            }
        }
        return;
    }
    if (state.kind === 'orphan-wal') {
        warnOnce(options.logger, `${dbPath}:post-close-orphan-wal`, `GitNexus: lbug.wal (${state.walBytes} bytes) remains without lbug.shadow after close; ` +
