 ...
 */
export function resolveNativeSafeStorageDir(storagePath, subdir) {
 ...
 */
let bufferPoolSizeHint;
/** Set (bytes) or clear (`undefined`) the per-run buffer-pool size hint. */
export const setBufferPoolSizeHint = (bytes) => {
 ...
 */
const resolveBufferManagerSize = () => {
 ...
 */
export const classifyDeleteAllError = (err) => {
    const msg = err instanceof Error ? err.message : String(err);
...
export function createLbugDatabase(lbugModule, databasePath, options = {}) {
    ...
    return new lbugModule.Database(databasePath, resolveBufferManagerSize(), // bufferManagerSize (#2557: default min(2 GiB, 80% RAM); GITNEXUS_LBUG_BUFFER_POOL_SIZE overrides; 0 restores the native 80%-of-RAM default)
