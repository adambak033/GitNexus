 ...
 */
const NATIVE_BUILD_PACKAGES = ['@ladybugdb/core', 'gitnexus', 'tree-sitter'];
 ...
 */
function restorePrebuiltNativeBinary(pkgDir, binaryPath) {
    try {
        ...
        try {
            ...
            const subPkg = createRequire(import.meta.url).resolve(`${manifest.name}-${process.platform}-${process.arch}/package.json`, { paths: [pkgDir] });
        ...
        candidates.push(path.join(pkgDir, 'prebuilt', `lbugjs-${process.platform}-${process.arch}.node`));
 ...
 */
function unrestorableBinaryFailure(outcome, pkgDir, binaryPath) {
    const lines = outcome === 'copy-failed'
        ? [
...
export function checkLbugNative(overridePkgDir) {
    let pkgDir;
    if (overridePkgDir) {
    ...
    if (restore !== 'restored')
        return unrestorableBinaryFailure(restore, pkgDir, binaryPath);
 ...
 */
function describeNativeLoadFailure(probe) {
    if (probe.signal) {
    ...
    return (errorLine?.replace(/^\w*Error:\s*/, '') ??
        lines.at(-1) ??
 ...
 */
export function glibcTooOldMessage(stderr) {
 ...
 */
export async function probeVectorExtensionLoad(timeoutMs = DEFAULT_FTS_PROBE_TIMEOUT_MS) {
