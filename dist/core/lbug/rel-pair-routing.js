 ...
 * would close a cycle.
 */
export const VALID_NODE_TABLES = new Set(NODE_TABLES);
/**
 ...
 */
export const relPairKeyFor = (fromId, toId, validTables) => {
 ...
 */
export const splitRelPairKey = (key) => {
 ...
 */
export const createRelationPairMatcher = () => /\bFROM\s+`?([A-Za-z][A-Za-z0-9_]*)`?\s+TO\s+`?([A-Za-z][A-Za-z0-9_]*)`?/g;
/**
 ...
 * COPY.
 */
export const parseRelationSchemaPairs = (relationSchema) => new Set([...relationSchema.matchAll(createRelationPairMatcher())].map((match) => `${match[1]}|${match[2]}`));
/**
 ...
 */
export const findUndeclaredRelationPairError = (err) => findInCauseChain(err, (e) => e instanceof UndeclaredRelationPairError);
/**
 ...
 */
export const assertDeclaredPair = (pairKey, declaredPairs, relationType, fromId, toId) => {
...
export class RelPairRouter {
    ...
    constructor(csvDir, header, validTables, declaredPairs, wsFactory = (p) => createWriteStream(p, 'utf-8')) {
        this.csvDir = csvDir;
        this.header = header;
