        ...
        try {
            ...
            if (!sidecarPreflightDisabled()) {
                try {
                    ...
                    logger.warn(`GitNexus: failed to reclaim missing-shadow WAL quarantines: ${summarizeError(err)}`);
                }
            }
 ...
 */
export const missingStagingCsvError = (table, csvPath, rows) => new Error(`Staging CSV for ${table} is missing: ${csvPath}. It was written with ` +
 ...
 * is genuinely gone.
 */
const stagingCsvExists = async (csvPath) => {
...
const copyNodeCSVs = async (targetConn, nodeFileEntries, log, totalSteps) => {
    ...
    for (const [table, { csvPath, rows }] of nodeFileEntries) {
        ...
        if (!(await stagingCsvExists(csvPath)))
            throw missingStagingCsvError(table, csvPath, rows);
    ...
    for (const [subBatchIndex, subBatch] of chunk(paramsList, SUB_BATCH_SIZE).entries()) {
        ...
        await withConnLock(async () => {
            ...
            try {
                ...
                throw new Error(`Batch execution failed for rows ${firstRow + 1}-${firstRow + subBatch.length}: ${msg} (${queryPreview})`);
            }
...
export const getLbugStats = async () => {
    ...
    let structuralEdges;
    let structuralEdgesError;
    try {
        const excluded = [...PDG_EDGE_TYPES].map((t) => `'${t}'`).join(', ');
        structuralEdges = await withConnLock(async () => {
            const queryResult = await c.query(`MATCH ()-[r:${REL_TABLE_NAME}]->() WHERE NOT r.type IN [${excluded}] RETURN count(r) AS cnt`);
        ...
        logger.warn({ err }, 'Structural relationship count failed; the graph-write-collapse check will have no ' +
    ...
    `;
        await withConnLock(async () => {
            ...
            try {
                ...
                logger.warn({ err }, `Incremental importer BFS: dropped chunk ${chunkIndex} (${batch.length} target path(s)) — ` +
...
const deleteAllRelationshipsOfType = async (relType, logTag, duplicateNoun, exactReasons) => {
    ...
    return withConnLock(async () => {
        ...
        try {
            ...
            const count = Number(rows[0]?.cnt ?? rows[0]?.[0] ?? 0);
            if (count > 0) {
                await closeQueryResults(await c.query(`MATCH ()-[r:CodeRelation]->() WHERE ${predicate} DELETE r`));
                edgesDeleted = count;
            }
        }
        catch (err) {
 ...
 */
export const deleteAllInterprocTaintPaths = async () => deleteAllRelationshipsOfType('TAINT_PATH', 'taint-interproc', 'duplicate cross-function findings');
/**
 ...
 */
export const deleteAllCallSummaries = async () => deleteAllRelationshipsOfType('CALL_SUMMARY', 'call-summary', 'duplicate summaries');
/**
 ...
 */
export const deleteAllInjects = async () => deleteAllRelationshipsOfType('INJECTS', 'di', 'duplicate INJECTS edges');
/**
 ...
 */
export const deleteAllAdvisedBy = async () => deleteAllRelationshipsOfType('ADVISED_BY', 'spring-aop', 'duplicate ADVISED_BY edges');
/** Drop all synthetic Spring AOP evidence nodes before incremental writeback. */
export const deleteSpringAopEvidenceNodes = async () => {
    const c = conn;
    if (!c) {
        throw new Error('LadybugDB not initialized. Call initLbug first.');
    }
    return withConnLock(async () => {
        ...
        try {
            ...
            const count = Number(rows[0]?.cnt ?? rows[0]?.[0] ?? 0);
            if (count > 0) {
                await closeQueryResults(await c.query(`MATCH (n:CodeElement) WHERE ${predicate} DETACH DELETE n`));
            }
            if (countResult)
                await closeQueryResults(countResult);
 ...
 */
export const deleteSpringAutoConfigurationDeclarations = async () => deleteAllRelationshipsOfType('DECLARES', 'spring-auto-configuration', 'duplicate auto-configuration declarations', SPRING_AUTO_CONFIGURATION_REASONS);
/**
 ...
 */
export const deleteSpringAutoConfigurationSyntheticClasses = async () => {
    const c = conn;
    if (!c) {
        throw new Error('LadybugDB not initialized. Call initLbug first.');
    }
    return withConnLock(async () => {
        ...
        try {
            ...
            const count = Number(rows[0]?.cnt ?? rows[0]?.[0] ?? 0);
            if (count > 0) {
                await closeQueryResults(await c.query(`MATCH (n:Class) WHERE ${predicate} DETACH DELETE n`));
            }
            if (countResult)
                await closeQueryResults(countResult);
            return { nodesDeleted: count };
 ...
 */
export const indexRowTable = (row) => row?.table_name ?? row?.[0];
export const indexRowName = (row) => row?.index_name ?? row?.[1];
export const indexRowType = (row) => row?.index_type ?? row?.[2];
/**
 ...
 */
export const readIndexCatalogRows = async () => {
    const targetConn = conn;
    if (!targetConn) {
        throw new Error('LadybugDB not initialized. Call initLbug first.');
    }
    try {
        return (await withConnLock(async () => readQueryRows(await targetConn.query('CALL SHOW_INDEXES() RETURN *'))));
    }
    catch (err) {
        logger.warn({ err }, 'Could not read the LadybugDB index catalog (CALL SHOW_INDEXES()); ' +
 ...
 */
export const INDEX_CATALOG_UNREADABLE = Symbol('gitnexus:index-catalog-unreadable');
/**
 ...
 */
export const readIndexCatalogSnapshot = async () => (await readIndexCatalogRows()) ?? INDEX_CATALOG_UNREADABLE;
/**
 ...
 */
export const resolveGateRows = async (indexRows) => {
 ...
 */
export const ensureEmbeddingRowDmlSafe = async (indexRows) => {
    ...
    const indexGatesDml = rows === undefined ||
        rows.some((row) => {
            if (indexRowTable(row) !== EMBEDDING_TABLE_NAME)
                return false;
 ...
 */
export const classifyFtsQueryError = (message) => {
    ...
    if (body.includes("doesn't have an index"))
...
export const buildFtsQueryCypher = (tableName, indexName, limit, conjunctive = false) => `
    CALL QUERY_FTS_INDEX('${tableName}', '${indexName}', $query, conjunctive := ${conjunctive})${tableName === 'Property' ? FTS_DETAIL_FILTER : ''}
    RETURN node, score
 ...
 */
export const isBenignDropFtsIndexError = (message) => message.startsWith('Binder exception:') || message.startsWith('Catalog exception:');
/**
 ...
 */
const ftsIndexPresenceInCatalog = async (tableName, indexName) => {
...
export const dropFTSIndex = async (tableName, indexName) => {
    ...
    try {
        ...
        if (presence !== 'absent') {
            ...
            throw new Error(`${lead} Every insert and delete against that table fails while the index is present. ` +
