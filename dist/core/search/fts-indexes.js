...
const formatWarningContext = (context) => {
...
export const ftsDegradedWarning = (context) => {
    ...
    return ('FTS indexes missing — keyword search degraded. Run: gitnexus analyze --repair-fts ' +
 ...
 */
export const ftsQueryFailedWarning = (context) => 'FTS keyword search failed — every configured index query returned an error' +
...
export async function dropSearchFTSIndexes(indexRows) {
    ...
    for (const { table, indexName } of FTS_INDEXES) {
        ...
        // creating.
        const provenAbsent = rows !== undefined &&
 ...
 */
export const summarizeFtsIndexBuildFailures = (failures) => `FTS index build failed for ${failures.length} of ${FTS_INDEXES.length} tables: ` +
...
const FTS_CAPABILITY_SIGNATURES = ['invalid utf-8', 'failed calling lower', 'tokeniz'];
 ...
 */
export const classifyFtsBuildError = (message) => {
    const m = message.toLowerCase();
    if (FTS_CAPABILITY_SIGNATURES.some((s) => m.includes(s)))
        return 'capability';
    if (FTS_INTEGRITY_SIGNATURES.some((s) => m.includes(s)))
        return 'integrity';
    return 'capability';
};
/**
 ...
 */
export const ftsFailureIsFatal = (failureClass, useAtomicSwap) => failureClass === 'integrity' && useAtomicSwap;
/**
 ...
 */
export async function buildSearchIndexesOrDegrade(executeQuery, options) {
    try {
