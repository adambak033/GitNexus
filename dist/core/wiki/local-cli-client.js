...
function verboseLog(provider, ...args) {
    if (isVerbose()) {
        logger.info({ provider, args }, '[local-cli]');
    }
}
const cachedCommands = new Map();
export function detectLocalCLI(provider) {
    if (cachedCommands.has(provider))
    ...
    try {
        ...
        if (!isNotFound && err instanceof Error) {
            logger.warn(`${provider} CLI found but --version failed (exit ${err.status ?? '?'}). ` +
...
export function resolveLocalCLIConfig(overrides) {
    return {
...
export async function callClaudeLLM(prompt, config, systemPrompt, options) {
...
export async function callCodexLLM(prompt, config, systemPrompt, options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (config.requestTimeoutMs !== undefined && config.requestTimeoutMs > 0) {
            killTimer = setTimeout(() => {
                killChildTree(child);
                const duration = config.requestTimeoutMs >= 60_000
                    ? `${Math.round(config.requestTimeoutMs / 60_000)}m`
                    : `${Math.round(config.requestTimeoutMs / 1_000)}s`;
                rejectOnce(new Error(`${provider} CLI timed out after ${duration}. ` +
...
function maskPromptArgs(provider, args) {
    if (provider === 'codex') {
        return args.map((arg) => (arg === '-' ? '[stdin prompt]' : arg));
    }
    return args;
}
function getDetectedCommand(provider) {
    detectLocalCLI(provider);
    return cachedCommands.get(provider) ?? null;
}
function resolveLocalCommand(provider) {
