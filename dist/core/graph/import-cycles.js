...
function buildGraph(edges) {
    ...
    for (const { source, target } of edges) {
        ...
        if (!targetsBySource.has(target))
            targetsBySource.set(target, new Set());
    }
 ...
 */
function overBudget(search, amount = 1) {
    search.work += amount;
    if (search.work <= search.workLimit)
        return false;
 ...
 */
function stronglyConnectedComponents(roots, adjacency, allowed, search) {
    const index = new Map();
    ...
    if (overBudget(search, roots.length))
        return components;
...
function isCyclic(component, selfLoops) {
    return component.length > 1 || selfLoops.has(component[0]);
}
function leastNode(nodes) {
    let least = nodes[0];
...
function byLeastNode(left, right) {
 ...
 */
function unblock(node, blocked, blockedBy) {
    const stack = [node];
 ...
 */
function enumerateCircuitsFrom(root, adjacency, allowed, search) {
    const blocked = new Set([root]);
 ...
 */
function enumerateComponentCycles(component, graph, search) {
    ...
    while (stack.length > 0 && search.exceeded === null) {
        ...
        if (overBudget(search, current.length))
            return;
        ...
        const decomposed = stronglyConnectedComponents(remaining, graph.adjacency, new Set(remaining), search);
 ...
 */
export function findImportCycles(edges, cycleLimit = IMPORT_CYCLE_LIMIT, workLimit = IMPORT_CYCLE_WORK_LIMIT) {
    const graph = buildGraph(edges);
    const allNodes = new Set(graph.nodes);
    const search = { cycles: [], cycleLimit, workLimit, work: 0, exceeded: null };
    ...
    if (search.exceeded !== null) {
        ...
        if (!decompositionComplete)
            return { enumeration: 'none', reason, limit };
