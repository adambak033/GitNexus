 ...
 */
export const CLASS_FRAMEWORK_ANNOTATIONS_FEATURE = {
    id: 'graph.class-framework-annotations',
    version: 1,
    appliesTo: () => true,
};
/** Resolve the exact feature set this build promises for the supplied files. */
export function resolveAnalysisFeatureVersions(descriptors, filePaths) {
    ...
    for (const descriptor of descriptors) {
        ...
        if (!Number.isSafeInteger(descriptor.version) || descriptor.version < 1) {
            throw new Error(`Analysis feature "${descriptor.id}" has invalid version ${descriptor.version}`);
        }
        if (seenIds.has(descriptor.id)) {
            throw new Error(`Duplicate analysis feature descriptor: ${descriptor.id}`);
        }
        seenIds.add(descriptor.id);
        if (!descriptor.appliesTo(filePaths))
            continue;
 ...
 */
export function findAnalysisFeatureMismatches(actual, expected) {
    const expectedKeys = Object.keys(expected);
    if (actual === undefined)
        return expectedKeys.map((id) => `missing:${id}`);
    if (actual === null || typeof actual !== 'object' || Array.isArray(actual)) {
        return ['invalid:analysisFeatures'];
    }
    const stamped = actual;
    const mismatches = [];
    for (const id of expectedKeys) {
        const value = stamped[id];
        if (value === undefined)
            mismatches.push(`missing:${id}`);
        else if (value !== expected[id])
            mismatches.push(`version:${id}`);
    }
